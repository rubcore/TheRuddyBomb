...
// ----------------------------------------------------------------------------------------------------
char *inttostr(char *dest, int integer) {
    if (integer == 0) {
        strcpy(dest, "0");
    } else {
        byte sign = 0;
...
char *fmt(char *dest, unsigned char argc, ...) {
    ...
    for (int i = 0; i < argc && buflen < LCD_COLS; i++) {
        str = va_arg(ap, char*);
        auto len = static_cast<unsigned char>(strlen(str));
        auto cpylen = static_cast<unsigned char>((buflen + len) > LCD_COLS ? LCD_COLS - buflen : len);
...
char *rpad(char *dest, const char *str, char chr, unsigned char width) {
    ...
    width = static_cast<unsigned char>(width > LCD_COLS ? LCD_COLS : width);
...
char *padc(char chr, unsigned char count) {
    ...
    count = static_cast<unsigned char>((count > LCD_COLS) ? LCD_COLS : count);
...
void buttonHandlerCycle() {
    if (millis() - buttonSampleTime >= 20) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (btnStateNow) {
                if ((millis() - buttonPressTime[i] > BUTTON_REPEAT_DELAY) &&
                    (millis() - buttonHoldTime[i] > BUTTON_REPEAT_SPEED_DELAY)) {
